...
const DepositAction: React.FC<React.PropsWithChildren<DepositActionProps>> = ({ totalValueLockedValue }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'POWER deposit will be diverted to the fixed-term staking pool. Please note that POWER deposited can ONLY be withdrawn after 10 weeks.',
    ),
    {
      placement: 'bottom',
    },
      ...
      <InputPanel>
        ...
        {isLessThanOneCake && (
          <Text color="failure" fontSize="14px" textAlign="right">
            {t('Please deposit at least 1 POWER to participate in the Pottery')}
          </Text>
        )}
      </InputPanel>
      <Flex>
        <Flex ml="auto">
          <Text fontSize="12px" color="textSubtle">
            {t('Deposited POWER will be locked for 10 weeks')}
          </Text>
          <Flex ref={targetRef}>
