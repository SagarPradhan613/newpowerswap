  ...
  return (
    <>
      <Box>
        ...
        <LightGreyCard>
          <BalanceRow title={t('POWER to be locked')} value={lockedAmount} newValue={newLockedAmount} decimals={2} />
          <BalanceRow title="iPOWER" decimals={2} value={formattediCake} newValue={newFormattediCake} />
          <BalanceRow
            title="apr"
            unit="%"
      ...
      {showLockWarning && (
        <Box mt="16px" maxWidth="370px">
          <Message variant="warning">
            <MessageText>
              {t('You will be able to withdraw the staked POWER and profit only when the staking position is unlocked')}
            </MessageText>
          </Message>
        </Box>
      )}
